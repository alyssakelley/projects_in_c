...
typedef struct {
	...
	char html_filenames[maxtopics][100]; // a 2D array of filenames (Note: this is only used when the thread_type_name is "subscriber")
...
void * create_html_page(int topic_num, int sub_id)
{
	/*
	...
	In my animal_mainfile.txt I add a publibsher, then subscriber 1 and subscriber 2 so they are in order, 
	but if I did add subscriber, then publisher, then subscriber, the subscriber numbers would be 0 and 2. 
	*/
	char filename[100];
	strcpy(filename, "Topic_");
	strcat(filename, Topics[topic_num].topic_name); // file will be in the formal of Topic_#.html
	strcat(filename, "_from_");
	strcat(filename, Thread_Pools.elements[sub_id].filename); // what sub is reading from
	strcat(filename, ".html");

	strcpy(Thread_Pools.elements[sub_id].html_filenames[topic_num], filename);
	printf("Created the HTML file for Topic #%d (%s) for Subscriber #%d and saved it as: %s\n", topic_num, Topics[topic_num].topic_name, sub_id, Thread_Pools.elements[sub_id].html_filenames[topic_num]);

	FILE * html_output = NULL;

	html_output = fopen(Thread_Pools.elements[sub_id].html_filenames[topic_num], "w+");

	if (html_output == NULL)
	{
		printf("Error creating HTML\n");
	}
	...
	fprintf(html_output, "\t<td align\"=left\">\n");
	fprintf(html_output, "\t\t<img SRC=\"puddles.png\" WIDTH=\"140\" HEIGHT=\"140\">&nbsp&nbsp&nbsp&nbsp&nbsp</a>\n");
	fprintf(html_output, "\t</td>\n");
	fprintf(html_output, "\t<td align=\"center\">\n");
	fprintf(html_output, "\t\t<h1>InstaQuack</h1>\n");
	fprintf(html_output, "\t\t<h1>Subscriber %d : Topic %s</h1>\n", sub_id, Topics[topic_num].topic_name);
	fprintf(html_output, "\t</td>\n");
	fprintf(html_output, "\t<td align=\"right\">\n");
	fprintf(html_output, "\t\t&nbsp&nbsp&nbsp&nbsp&nbsp<img SRC=\"puddles.png\" WIDTH=\"140\" HEIGHT=\"140\"></a>\n");
	fprintf(html_output, "\t</td>\n");
	fprintf(html_output, "</tr>\n");
	fprintf(html_output, "</table>\n");
...
void * fill_in_html_body(int topic_num, int entrynum, int sub_id)
{
	/*
	...
	*/
	...
	if ( strcmp(Topics[topic_num].Queue[entrynum].photoUrl, "NULL") == 0)
	{
		// printf("Oops its NULL\n");
	}
	else
	{
		FILE * html_output = NULL;

		html_output = fopen(Thread_Pools.elements[sub_id].html_filenames[topic_num], "a");
		...
		fprintf(html_output, "<hr>\n");

		char url[300];
		strcpy(url, Topics[topic_num].Queue[entrynum].photoUrl);
		url[(strlen(url) - 1)] = 0;

		fprintf(html_output, "<img SRC=\"%s\"</a><br>\n", url);
		...
		printf("\n\n{Filling in the HTML file for this entry}\n\tAdding to file: %s \n\tTopic # %d (%s) - Entry #%d contents:\n\t\tURL = %s \n\t\tCAPTION = %s\n\n", Thread_Pools.elements[sub_id].html_filenames[topic_num], topic_num, Topics[topic_num].topic_name, entrynum, url, caption);
		fclose(html_output);
	}
	return 0;
}

void * end_html_body(int topic_num, int sub_id)
{
	/*
	...
	*/
	...
	fprintf(html_output, "</html>\n");
	fclose(html_output);
	printf("Done creating the html file: %s\n", Thread_Pools.elements[sub_id].html_filenames[topic_num]);
	return 0;
}
...
int sub(int topic_num, int lastentry, int sub_num)
{
	...
	returned, then you can see the fill_in_html_body() is called to put that last read entry in the HTML file for
	this sub_num subscriber, and then last read entry is incremented so it can try to read the next one next.
	If found is -1, the topic queue is empty, and if it is -100 then that means the last read entry is not valid
	because that number is not enqueued yet. In those two cases, fill_in_html_body() is not called because you cannot
	view an entry. If it is anything other than -1, -100, or 1, this means that you cannot read the last read entry, 
	but you can read the next available entry in the topic queue and that is the number that is returned. You call
	the fill_in_html_body() in this case so you can put that next available viewable topic in the HTML for this
	sub_num subscriber thread. The next last read entry is returned (but also updated in the Topic struct).
	*/
	int found = 0;
	...
	else if (found == 1)
	{
		...
		fill_in_html_body(topic_num, lastentry, sub_num);
		lastentry++;
...
void * cleanup_thread(void * data)
{
	...
	*/
	...
	while(EXIT_VAR == 0)
	{
		...
		for (i = 0; i <= maxtopics; i++)
		{
			struct timeval time; // PUT THIS LOWER?
			gettimeofday(&time, NULL);
	...
	*/
	...
	for(i = 0; i < ACTUAL_PROXY_AMT; i++)
	{
		...
		else if (strcmp(Thread_Pools.elements[i].thread_type_name, "subscriber") == 0)
		{
			...
			int j;
			for(j = 0; j < ACTUAL_TOPIC_AMT; j++)
			{
				create_html_page(j, curr_sub_id);
			}
...
int main(int argc, char* argv[])
{
	...
	for (thread_num = 0; thread_num < ACTUAL_PROXY_AMT; thread_num++) // looping through all the subscriber threads and then all the topics to end each HTML file, this could also be done in the subscriber function at the end.
	{
		for(topic = 0; topic < ACTUAL_TOPIC_AMT; topic++)
		{
			if (strcmp(Thread_Pools.elements[thread_num].thread_type_name, "subscriber") == 0)
			{
				end_html_body(topic, thread_num);
			}
		}
	}
