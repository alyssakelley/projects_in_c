...
void parse_the_line(Process * command_line, char * line)
{
	...
	*/
	...
	command_line->current_command = command_name;//malloc(sizeof(char) * length_of_command);

	// ************* MAY NEED TO ADD BELOW LINE BACK IN!
	command_line->args[0] = command_name;

	int counter = 1; // may need to change this back to one!!
	...
	while(current_arg != NULL)
	{
		...
		//strcpy(command_line -> args [counter], current_arg);
		current_arg = strtok(NULL, " ");
		counter++;
	}

	//line_count++;
	//command_line[line_count].num = arg_count;
	//free(copy_of_line); // ?
}
